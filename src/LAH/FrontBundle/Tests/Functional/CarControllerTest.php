...
use LAH\FrontBundle\Dictionary\Country;
use LAH\FrontBundle\Dictionary\Engine;
use LAH\FrontBundle\Dictionary\Fuel;
use LAH\FrontBundle\Dictionary\Gearbox;
...
class CarControllerTest extends BaseFunctionalTest
{
    ...
    public function car_add_form_renders_properly()
    {
        ...
        $forms = $crawler->filter('form[name="lah_form_car"]');
        ...
        $this->assertCount(count($carTypes) + 1, $crawler->filter('select#lah_form_car_type option'));
        $this->assertCount(count($carModels) + 1, $crawler->filter('select#lah_form_car_model option'));
        $this->assertCount(count($carColors) + 1, $crawler->filter('select#lah_form_car_color option'));
        $this->assertCount(count(Fuel::$choicesArray) + 1, $crawler->filter('select#lah_form_car_fuel option'));
        $this->assertCount(count(Engine::$choicesArray) + 1, $crawler->filter('select#lah_form_car_engine option'));
        $this->assertCount(count(Gearbox::$choicesArray) + 1, $crawler->filter('select#lah_form_car_gearbox option'));
        $this->assertCount(count(Country::$choicesArray) + 1, $crawler->filter('select#lah_form_car_registrationCountry option'));
    }
    ...
    public function car_add_form_is_posted_and_entity_is_added()
    {
        ...
        $form->setValues([
            'lah_form_car[type]'                                                => 1,
            'lah_form_car[model]'                                               => 1,
            'lah_form_car[color]'                                               => 1,
            'lah_form_car[year]'                                                => 1990,
            'lah_form_car[distance]'                                            => 40000,
            'lah_form_car[fuel]'                                                => Fuel::DIESEL,
            'lah_form_car[engine]'                                              => Engine::DIESEL,
            'lah_form_car[gearbox]'                                             => Gearbox::AUTOMATIC,
            'lah_form_car[registrationCountry]'                                 => Country::POLAND,
            'lah_form_car[promoteCarousel]'                                     => true,
            'lah_form_car[promoteFrontpage]'                                    => false,
            'lah_form_car[translations]['.$this->defaultLocale.'][title]'       => $title,
            'lah_form_car[translations]['.$this->defaultLocale.'][description]' => 'Some description',
            'lah_form_car[adminNotes]'                                          => 'Admin Notes',
            'lah_form_car[hidden]'                                              => false,
        ]);

        $values                             = $form->getPhpValues();
        $values['lah_form_car']['prices'] = [
    ...
    public function car_edit_form_is_posted_and_entity_is_changed()
    {
        ...
        $form                                                                    = $forms->first()->form();
        $values                                                                  = $form->getPhpValues();
        $values['lah_form_car']['translations'][$this->defaultLocale]['title'] = $title;
        $values['lah_form_car']['prices']                                      = [
            ['amount' => 30, 'currency' => 2],
            ['amount' => 40, 'currency' => 2],
        ];
        $newModelName                                   = md5('some title'.microtime());
        $values['lah_form_car']['add_model']['name']  = $newModelName;
        $values['lah_form_car']['add_model']['brand'] = $newModelName;

        $files                             = [];
        $files['lah_form_car']['images'] = $this->getUploadedImages(2);
    ...
    public function car_add_form_is_posted_and_error_is_emitted($formData, $collections, $error)
    {
        ...
        $values                             = $form->getPhpValues();
        $values['lah_form_car']['prices'] = $collections['prices'];

        $crawler = $this->client->request('POST', $form->getUri(), $values, ['lah_form_car' => ['images' => $collections['images']]]);
    ...
    public function new_type_is_being_created_while_editing_new_car()
    {
        ...
        $form                                         = $forms->first()->form();
        $values                                       = $form->getPhpValues();
        $values['lah_form_car']['add_type']['name'] = $newTypeName;
        $values['lah_form_car']['prices']           = $this->getPrices(1);

        $files                             = [];
        $files['lah_form_car']['images'] = $this->getUploadedImages(count($carEntity->getImages()));
    ...
    public function new_color_is_being_created_while_adding_new_car()
    {
        ...
        $form->setValues([
            ...
            'lah_form_car[translations]['.$this->defaultLocale.'][description]'     => 'Some description',
            'lah_form_car[hidden]'                                                  => false,
            'lah_form_car[adminNotes]'                                              => 'notes',
            'lah_form_car[add_color][translations]['.$this->defaultLocale.'][name]' => $newColorName,
        ]);

        $files                             = [];
        $files['lah_form_car']['images'] = $this->getUploadedImages(1);
    ...
    public function new_color_is_being_created_while_editing_new_car()
    {
        ...
        $form                                                                                = $forms->first()->form();
        $values                                                                              = $form->getPhpValues();
        $values['lah_form_car']['add_color']['translations'][$this->defaultLocale]['name'] = $newColorName;
        $values['lah_form_car']['prices']                                                  = $this->getPrices(1);
    ...
    public function new_model_is_being_created_while_adding_new_car()
    {
        ...
        $form->setValues([
            ...
            'lah_form_car[hidden]'                                              => false,
            'lah_form_car[adminNotes]'                                          => 'notes',
            'lah_form_car[add_model][name]'                                     => $newModelName,
            'lah_form_car[add_model][brand]'                                    => $newModelName,
        ]);
    ...
    public function new_model_type_color_brand_are_not_created_but_reused_when_adding_car()
    {
        ...
        $form->setValues([
            ...
            'lah_form_car[hidden]'                                                  => false,
            'lah_form_car[adminNotes]'                                              => 'notes',
            'lah_form_car[add_model][name]'                                         => $model->getName(),
            'lah_form_car[add_model][brand]'                                        => $brand->getName(),
            'lah_form_car[add_color][translations]['.$this->defaultLocale.'][name]' => $color->getName(),
            'lah_form_car[add_type][name]'                                          => $type->getName(),
        ]);
    ...
    public function new_model_type_color_brand_are_not_created_but_reused_when_editing_car()
    {
        ...
        $values['lah_form_car']['translations'][$this->defaultLocale]['title']             = $title;
        $values['lah_form_car']['add_model']['name']                                       = $model->getName();
        $values['lah_form_car']['add_model']['brand']                                      = $brand->getName();
        $values['lah_form_car']['add_color']['translations'][$this->defaultLocale]['name'] = $color->getName();
        $values['lah_form_car']['add_type']['name']                                        = $type->getName();
        $values['lah_form_car']['prices']                                                  = $this->getPrices(1);
    ...
    public function getErrorGeneratingForms()
    {
        $correctData = [
            ...
            'lah_form_car[promoteCarousel]'                                     => true,
            'lah_form_car[promoteFrontpage]'                                    => false,
            'lah_form_car[translations]['.$this->defaultLocale.'][title]'       => 'Some title',
            'lah_form_car[translations]['.$this->defaultLocale.'][description]' => 'Some description',
            'lah_form_car[adminNotes]'                                          => 'Admin Notes',
            'lah_form_car[hidden]'                                              => false,
        ];
        ...
        return [
            ...
            'empty brand' => [
                'formData' => array_merge(
                    $correctData,
                    [
                        'lah_form_car[add_model][name]'  => 'someName',
                        'lah_form_car[add_model][brand]' => null,
                    ]
                ),
                'collections' => $collections,
            ...
            'empty model, filled brand' => [
                'formData' => array_merge(
                    $correctData,
                    [
                        'lah_form_car[add_model][name]'  => null,
                        'lah_form_car[add_model][brand]' => 'brand',
                    ]
                ),
                'collections' => $collections,
