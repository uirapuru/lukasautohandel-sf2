...
final class FeatureContext extends MinkContext implements Context
{
    ...
    public function iAddImages($amount)
    {
        ...
        $page   = $this->getSession()->getPage();
        $button = $page->find('css', 'div#lah_car_images');

        if (count($button) == 0) {
            throw new Exception('Element div#lah_car_images not found!');
        }
        ...
        $this->assertNumElements(5, 'div#lah_car_images input[type=file]');
    }
    ...
    public function iRemoveImages($amount)
    {
        $amount = $this->fixStepArgument($amount);
        $page   = $this->getSession()->getPage();
        $row    = $page->find('css', 'div#lah_car_images')->getParent();
    ...
    {
        $this->assertNumElements(0, 'div#lah_car_prices > ul > li');
    }
    ...
    public function iAddPrices($amount)
    {
        ...
        $button = $page->find('css', 'div#lah_car_prices');

        if (count($button) == 0) {
            throw new Exception('Element div#lah_car_prices not found!');
        }
        ...
        $this->assertNumElements($amount, 'div#lah_car_prices > ul > li');
    }
    ...
    public function iRemovePrices($amount)
    {
        $amount = $this->fixStepArgument($amount);
        $page   = $this->getSession()->getPage();
        $row    = $page->find('css', 'div#lah_car_prices')->getParent();
        $links  = array_slice($row->findAll('css', 'a.item_remove'), 0, $amount);
    ...
    {
        $this->assertNumElements($arg1, 'div#lah_car_prices > ul > li');
    }
    ...
    {
        $page   = $this->getSession()->getPage();
        $button = $page->find('css', sprintf('div#lah_car_prices_%d button.dropdown-toggle', $number - 1));
        ...
        $link = $page->find(
            'css',
            sprintf(
                'div#lah_car_prices_%d ul.dropdown-menu a.changeCurrencyLink[data-code="%s"]',
                $number - 1,
                $value
            )
        );
    ...
    {
        $page   = $this->getSession()->getPage();
        $button = $page->find('css', sprintf('div#lah_car_prices_%d button.currencySymbol', $number - 1));
